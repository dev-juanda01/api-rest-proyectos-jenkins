pipeline {
    ...
    stages {
        stage('Clonar el Repositorio'){
            steps {
                git branch: 'main', credentialsId: 'git-jenkins', url: 'https://github.com/dev-juanda01/api-rest-proyectos-jenkins.git'
            }
        }
            ...
            steps {
                script {
                    withCredentials([
                        ...
                        docker.build('api-rest-proyectos-micro', '--build-arg MONGO_URI=${MONGO_URI} .')
                    }
                }
            }
        }
